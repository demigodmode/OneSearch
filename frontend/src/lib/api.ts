 ...
 */
export async function clearStaleFailed(
  id: string,
): Promise<{
  cleared: number;
  reindexed: number;
  ...
  return apiFetch<{
    ...
    still_failed: number;
    skipped: number;
  }>(`/sources/${encodeURIComponent(id)}/clear-stale`, { method: "POST" });
}
...
export async function getAgent(id: string): Promise<AgentDetails> {
  return apiFetch<AgentDetails>(`/agents/${encodeURIComponent(id)}`);
}
export async function createAgentEnrollment(): Promise<AgentEnrollment> {
  return apiFetch<AgentEnrollment>("/agents/enrollments", { method: "POST" });
}
export async function approveAgent(id: string): Promise<Agent> {
  return apiFetch<Agent>(`/agents/${encodeURIComponent(id)}/approve`, {
    method: "POST",
  });
}
export async function disableAgent(id: string): Promise<Agent> {
  return apiFetch<Agent>(`/agents/${encodeURIComponent(id)}/disable`, {
    method: "POST",
  });
}
export async function revokeAgent(id: string): Promise<Agent> {
  return apiFetch<Agent>(`/agents/${encodeURIComponent(id)}/revoke`, {
    method: "POST",
  });
}
export async function updateAgentProcessingMode(
  id: string,
  default_processing_mode: ProcessingMode,
): Promise<Agent> {
  return apiFetch<Agent>(`/agents/${encodeURIComponent(id)}`, {
    method: "PATCH",
    body: JSON.stringify({ default_processing_mode }),
  });
}
...
export async function getDocumentDownloadLink(
  ...
  return apiFetch<DocumentDownloadLink>(
    `/documents/${encodeURIComponent(id)}/download-link`,
    {
      method: "POST",
    },
  );
