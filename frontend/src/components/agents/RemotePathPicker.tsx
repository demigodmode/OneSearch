...
export function RemotePathPicker({ agent, value, onChange, onTest, result, testing, onBrowse }: {
  ...
  const browse = (rootId: string, path: string): Promise<BrowsePage> => {
    ...
    return onBrowse({ agent_id: agent.id, root_id: rootId, path })
      .then((response) => (
        // don't throw here: FolderBrowser must check root/path identity before showing a failure, same order as before
  ...
  return <div className="space-y-2">
    ...
    <FolderBrowser key={browserKey} roots={agent?.allowed_roots ?? []} available={connected} pathStyle={pathStyle} browse={browse} onSelect={onChange} />
