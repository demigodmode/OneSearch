...
def _response(
    ...
    return AgentAdminResponse(
        ...
        update_report=(None if agent.update_status is None else AgentAdminUpdateReport(
            auto_update=agent.auto_update, runtime_kind=agent.update_runtime_kind,
            status=agent.update_status, available_version=agent.update_available_version,
            checked_at=agent.update_checked_at, error_code=agent.update_error_code,
        )),
